...
def clean_price_data(ticker1, ticker2):
    ...
    # Make sure the series have the same length by matching dates
...
#We are interested in how close the price ratio is to 1
...
def magnitude_squared_coherence(x, y):
    # Trim the longer time series to the length of the shorter one
    max_len = max(len(x), len(y))

    # Pad the shorter time series with zeros
    x_padded = np.pad(x, (0, max_len - len(x)))
    y_padded = np.pad(y, (0, max_len - len(y)))

    # Assuming evenly spaced samples
    fs = 1 / np.mean(np.diff(np.arange(max_len)))

    # Calculate cross-power spectral density at frequency zero
    _, Pxy = csd(x_padded, y_padded, fs=fs, nperseg=max_len)
    CSD_at_zero = Pxy[0]

    # Calculate power spectral densities at frequency zero
    _, Pxx = csd(x_padded, x_padded, fs=fs, nperseg=max_len)
    _, Pyy = csd(y_padded, y_padded, fs=fs, nperseg=max_len)

    # Calculate magnitude-squared coherence at frequency zero
...
def rank_pairs(pair_dict):
    # Extract pairs and measures from the input dictionary
    pairs = list(pair_dict.keys())
    measures = list(pair_dict[pairs[0]].keys())

    # Initialize a dictionary to store ranks for each parameter
    ranks = {param: np.argsort([pair_dict[pair][param] for pair in pairs]) + 1 for param in measures}

    # Initialize a dictionary to store the total score for each pair
    total_ranks = {pair: 0 for pair in pairs}

    # Define the ranking criteria for each parameter
    ranking_criteria = {
    ...
    # Calculate the total rank for each pair based on individual parameter ranks
